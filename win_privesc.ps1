...
function LootingForInterrestingFile{
  ...
  Get-ChildItem -Path C:\ | Where-Object{$_.Name -ne "Windows" -and  $_.Name -ne "Program Files" -and $_.Name -ne "Program Files (x86)"} | ForEach-Object{ Get-ChildItem -Path $_.FullName -Recurse -File  -Include '*.txt','*.py','*.php','*.ps1','*.bat','*.bak','*.ini','*.config','*.aspx','*.asp','*.csv','*.hta','*.vbs','*.vba','*.java' -ErrorAction SilentlyContinue  | Select-String -Pattern 'user', 'login', 'pass', 'pwd', 'psw', 'database', 'secret', 'password', 'vnc'  -AllMatches -ErrorAction SilentlyContinue | Select-Object -Unique Path }
 
  Get-ChildItem -Path "C:\Program Files" | Where-Object{$_.Name -ne "Common Files" -and  $_.Name -ne "internet explorer" -and $_.Name -NotLike "Microsoft*" -and $_.Name -ne "VMware" -and $_.Name -NotLike "Windows*"} | ForEach-Object{ Get-ChildItem -Path $_.FullName -Recurse -File  -Include '*.txt','*.py','*.php','*.ps1','*.bat','*.ini','*.config','*.bak','*.aspx','*.asp','*.csv','*.hta','*.vbs','*.vba','*.java' -ErrorAction SilentlyContinue  | Select-String -Pattern 'user', 'login', 'pass', 'pwd', 'psw', 'database', 'secret', 'password', 'vnc'  -AllMatches -ErrorAction SilentlyContinue | Select-Object -Unique Path }
 
  Get-ChildItem -Path "C:\Program Files (x86)" | Where-Object{$_.Name -ne "Common Files" -and  $_.Name -ne "internet explorer" -and $_.Name -NotLike "Microsoft*" -and $_.Name -ne "VMware" -and $_.Name -NotLike "Windows*"} | ForEach-Object{ Get-ChildItem -Path $_.FullName -Recurse -File  -Include '*.txt','*.py','*.php','*.ps1','*.bat','*.config','*.bak','*.ini','*.aspx','*.asp','*.csv','*.hta','*.vbs','*.vba','*.java' -ErrorAction SilentlyContinue  | Select-String -Pattern 'user', 'login', 'pass', 'pwd', 'psw', 'database', 'secret', 'password', 'vnc'  -AllMatches -ErrorAction SilentlyContinue | Select-Object -Unique Path }
